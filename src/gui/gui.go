	...
	for {
		...
		if opcode == 0x03 {
			...
			if err := json.NewDecoder(reader).Decode(&state); err == nil {
				...
				for _, id := range state.RoNpcsTalk { ui.CurrentPlayer.Room.Exist[id] = "TALK" }
				for _, id := range state.RoNpcsHostil { ui.CurrentPlayer.Room.Exist[id] = "HOSTILE" }
...
func (ui *GameUI) setupLayout() {
    ...
    movePanel := container.NewVBox(
        container.NewHBox(widget.NewButton("N", func() { ui.sendCommand("MOVE NORTH") }), widget.NewButton("E", func() { ui.sendCommand("MOVE EAST") })),
        container.NewHBox(widget.NewButton("W", func() { ui.sendCommand("MOVE WEST") }), widget.NewButton("S", func() { ui.sendCommand("MOVE SOUTH") })),
    )

    leftSide := container.NewBorder(nil, container.NewPadded(movePanel), nil, nil, container.NewScroll(ui.MudConsole))
